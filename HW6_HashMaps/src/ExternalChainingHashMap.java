...
public class ExternalChainingHashMap<K, V> {
    ...
    public V put(K key, V value) {
        ...
        ExternalChainingMapEntry<K, V> curr = table[addIndex];
        if (curr == null) {
            ...
            newNode.setNext(table[addIndex]);
            table[addIndex] = newNode;
        }
        size++;
        return null;
    }
    ...
    public V remove(K key) {
        ...
        ExternalChainingMapEntry<K, V> curr = table[removeIndex];
        if (curr != null && curr.getKey().equals(key)) {
            output = curr.getValue();
            table[removeIndex] = curr.getNext();
            size--;
            return output;
        } else if (curr != null) {
            while (curr.getNext() != null) {
                if (curr.getNext().getKey().equals(key)) {
                    output = curr.getNext().getValue();
    ...
    public V get(K key) {
        ...
        ExternalChainingMapEntry<K, V> iterator = table[keyIndex];
        while (iterator != null) {
     ...
     */
    public Set<K> keySet() {
        Set<K> output = new HashSet<K>(size, (float) MAX_LOAD_FACTOR);
        for (int i = 0; i < table.length; i++) {
            if (table[i] != null) {
                output.add(table[i].getKey());
    ...
    public void resizeBackingTable(int length) {
        if (length < size) {
            throw new IllegalArgumentException("Length is smaller than size!");
        }
        ...
        for (int i = 0; i < table.length; i++) {
            if (table[i] != null) {
                ...
                while (curr.getNext() != null) {
                    curr = curr.getNext();
                    index = Math.abs(curr.getKey().hashCode() % newTable.length);
                    newTable[index] = curr;
                }
            }
        }
